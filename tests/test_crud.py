...
async def test_get_recent_bars_success():
    ...
    mock_result.scalars.return_value.all.return_value = [
        PriceBar(symbol='ethusdt', timestamp=datetime.now(), open=2500.0, high=2510.0, low=2490.0, close=2505.0,
                 volume=1000.0)
    ]
...
async def test_cleanup_old_data_success():
    ...
    bars_deleted, results_deleted, alerts_deleted = await CRUDOperations.cleanup_old_data(
        session=mock_session,
        days_to_keep=7
    )
